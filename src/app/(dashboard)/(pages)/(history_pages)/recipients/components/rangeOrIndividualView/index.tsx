...
const FormRangeView = ({ setRange, ranges, label, error, value, inRange = true }: IRangeOrIndividualView): JSX.Element => {
  return (
    <section className={`rangeOrIndividualView ${acl(!!error, 'box-error')}`}>
      ...
      <section className='rangeOrIndividualView-range'>
        {ranges.map(r => {
          const isActive = inRange ? value.some((range: any) => range >= r[0] && range <= r[1]) : value === r
          return (
            <button key={inRange ? r.join(' - ') : r} className={acl(isActive)} type='button' onClick={() => setRange(r)}>
              {inRange ? r.join(' - ') : r}
            </button>
          )
        })}
