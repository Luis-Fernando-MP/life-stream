...
const scheme = z.object({
  ...
  lastName: z
    ...
    .max(100, { message: 'Máximo 100 caracteres' })
    .regex(/^[a-zA-ZáéíóúÁÉÍÓÚ\s]*$/, 'Sin números o caracteres especiales'),
  dni: z.string({ message: 'El DNI es requerido' }).length(8, { message: 'DNI de 8 dígitos' }).regex(/^\d*$/, 'Solo números'),
  photo: z.string({ message: 'La foto es requerida' })
})

export const receptorRegisterResolver = zodResolver(scheme)
export type IReceptorRegisterResolver = z.infer<typeof scheme>
